...
class Drone1DEnv():
    ...
    def __init__(self, drone, marker,phi_des):
        ...
        self.phi_dot_high = np.deg2rad(500)
        self.phi_des = phi_des
        self.phi_dot_des = 0.0
        self.action_high = 3.2 #(N-m)
    ...
    def reward(self, action):
        ...
        if self.done():
            ...
            reward = -(20*abs(phi) + 2*abs(phi_dot) + 0.1*abs(action) + 0.5*t)
